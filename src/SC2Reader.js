...
import { SMod } from './schema/mod.js';
export const DATA_FILES = EGameCatalog.enum


class SC2ComponentsReader extends VFS {
    configs = {
    ...
    constructor(options){
        ...
        let isNode = !!process?.versions?.node
        ...
        this.addReaders(
            ...
            {prefix: "zip:", reader:cs.Zip, name: "zip"}
        )
    }
    ...
    //         if(reader.base !== undefined)reader.base = base
    //     }
    // }

    // setDatabase(dbName) {
    //     for(let reader of this.readers){
    //         if(reader.dbname !== undefined)reader.dbname = dbName
    //     }
    // }
     ...
     */
    async readModData( modName, options = {}) {
        const defaultScope = {
            catalogs: true,
            strings: true,
            locales: true,
        ...
        // Parse catalogs
        if (scope.catalogs) {
            const baseCatalogs = DATA_FILES.map(f => "gamedata/" + f.toLowerCase() + "data.xml");
            ...
            loadedData.catalogs = await this.mapToObject(
            allCatalogs.filter(f => files.includes(f)),
            file => Parsers.xml(file,SMod.catalogs[0]),
            true
            );
        ...
        if (scope.layouts) {
            ...
            const layoutsRaw = await this.mapToObject(descIndexIncludes.map(f => "Base.SC2Data/" + f), Parsers.xml,SMod.layouts[0]);
        ...
        // Triggers
        if (scope.triggers) {
            loadedData.sc2lib = await this.mapToObject(files.filter(f => /\.sc2lib$/i.test(f)), Parsers.xml,SMod.triggers[0]);
        }
        ...
        // Cutscenes
        if (scope.cutscenes) {
            loadedData.cutscenes = await this.mapToObject( files.filter(f => /\.(sc2cutscene|stormcutscene)$/i.test(f)), Parsers.xml,SMod.cutscenes[0]);
        }
    ...
    async writeModData( modName, obj , options) {

        await this.init(modName);
        ...
        for(let file in filesData){
            console.log(file)
            await this.set(file,filesData[file])
        }

        for(let file in binary){
            await this.set(file,filesData[file],{file: true})
        }
    }


    async write(modName, modData, options){
        await this.writeModData(modName,modData,options);
    }

    async read(mod, options){
        let modData = await this.readModData(mod,options);
...
const SC2Reader = new SC2ComponentsReader();
export default SC2Reader;

SC2Mod.prototype.write = function(modName, options){
    SC2Reader.write(modName, this, options)
}