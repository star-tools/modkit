...
import {SCSchema,getSchema,relations} from "./schema/all.js"
...
//recoursive resolving of tokens. 
  ...
  _calculate_object_tokens(obj,entity){
    objectDeepTransform(obj, val => val && val.constructor === String && val.includes("##"), null, ({val, obj, prop, id, trace}) => {
  ...
  _calculate_value(entity,field){
    ...
    let patched = applyArrayPatches(resolved)
...
//calculates expression values `Behavior,ZerglingArmorShredTarget,Duration`
    ...
    if(  this.catalogs){
      ...
      for(let catalog of   this.catalogs){
        ...
        if(structCatalog.length){
          ...
          this.catalogs.push(...Object.entries(outputCatalogs).map(([name,data]) => ({
  ...
  calculateAllValues(entity){
    let result = {}
    for(let parent of entity.parents) objectsDeepMerge(result,parent.data)
    objectsDeepMerge(result,entity.data)
    result.default = entity.data.default
    if(!entity.data.default)delete result.default
    result.comment = entity.data.comment
    if(entity.data.comment === undefined)delete result.comment
    delete result.parent
    let resultPatched = applyArrayPatches(result)
    this._calculate_object_tokens(resultPatched)
    entity.calculated = result
    return result
  }
